...
class PDFController
  ...
  def bates
    @params = Plezi.rubyfy(params)
    # redirect back to the form page if there's no posted data.
    return response.redirect '/' unless params[:file] && params[:file][:'0']
    params[:file] = params[:file].values
    ...
    begin
      ...
      if !completed_pdf.pages.empty?
        ...
        send_data completed_pdf.to_pdf, filename: output_name
        ...
        cookies[:notice] = 'Unknown error - 0 pages.'
        response.redirect '/'
      end
      ...
      cookies[:notice] = "Unsupported or error reading file:\n#{file_name}"
      response.redirect '/'
      return true
    end
  end
  ...
  def code
    response.redirect 'https://github.com/boazsegev/combine_pdf_demo/blob/c9914588e4116dcfdaa37f85727f442b064e2b04/pdf_controller.rb'
  end
end
